...
- The generated report is a single static article. Nested pages render as
  linked inline sections with stable anchors.
...
def _render_site_html(pages: list[ReportPage], manifest: dict[str, object]) -> str:
    ordered_pages = sorted(pages, key=lambda p: (p.relative_path != "index.md", p.relative_path))
    root_page = next((page for page in ordered_pages if page.relative_path == "index.md"), ordered_pages[0])
    child_pages = [page for page in ordered_pages if page.relative_path != root_page.relative_path]
    title = html.escape(root_page.title)
    linked_pages = _render_linked_pages(child_pages)
    child_sections = "\n".join(
        f"""<section class="report-section" id="{html.escape(_page_anchor(page.relative_path))}" data-report-page="{html.escape(page.relative_path)}">
  <div class="section-label">{html.escape(page.relative_path)}</div>
  {page.html}
</section>"""
...
<head>
  ...
  <title>{title}</title>
  <style>
    :root {{ color-scheme: light; --paper: #fffdf8; --bg: #f2efe8; --ink: #1d252f; --muted: #667085; --line: #d9d2c3; --soft: #f7f2e8; --accent: #2f5f8f; --accent-soft: #e8f0f7; --code: #eee7da; }}
    * {{ box-sizing: border-box; }}
    html {{ scroll-behavior: smooth; }}
    body {{ margin: 0; color: var(--ink); background: radial-gradient(circle at top, #fbf7ef 0, var(--bg) 440px); font-family: ui-serif, Georgia, Cambria, "Times New Roman", serif; }}
    a {{ color: var(--accent); text-decoration-thickness: 1px; text-underline-offset: 3px; }}
    .article-shell {{ max-width: 1040px; margin: 0 auto; padding: 56px 24px 96px; }}
    .article-paper {{ background: var(--paper); border: 1px solid var(--line); box-shadow: 0 20px 60px rgba(74, 58, 31, 0.12); padding: 64px min(8vw, 88px); }}
    .article-header {{ border-bottom: 1px solid var(--line); padding-bottom: 28px; margin-bottom: 34px; }}
    .eyebrow {{ color: var(--accent); font: 700 12px/1.3 ui-sans-serif, system-ui, sans-serif; letter-spacing: 0.08em; text-transform: uppercase; margin-bottom: 18px; }}
    .article-title {{ font-size: clamp(36px, 6vw, 64px); line-height: 0.98; margin: 0 0 18px; font-weight: 700; letter-spacing: 0; }}
    .article-meta {{ color: var(--muted); font: 14px/1.6 ui-sans-serif, system-ui, sans-serif; display: flex; flex-wrap: wrap; gap: 10px 18px; }}
    .root-content > h1:first-child {{ display: none; }}
    .abstract {{ background: var(--soft); border: 1px solid var(--line); padding: 20px 24px; margin: 0 0 30px; }}
    .abstract h2 {{ border: 0; padding: 0; margin-top: 0; font-size: 20px; }}
    ...
    .linked-pages {{ margin: 34px 0 42px; padding: 24px; background: var(--soft); border: 1px solid var(--line); }}
    ...
    .report-section {{ margin-top: 56px; padding-top: 12px; }}
    .section-label {{ color: var(--muted); font: 12px/1.4 ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; margin-bottom: 8px; }}
    ...
    .agent-note {{ margin-top: 8px; padding: 12px 14px; background: #eef5f8; border-left: 3px solid var(--accent); color: #344054; font: 13px/1.55 ui-sans-serif, system-ui, sans-serif; }}
    .agent-note code {{ background: rgba(255,255,255,0.72); font-size: 12px; }}
    ...
    @media (max-width: 760px) {{ .article-shell {{ padding: 20px 12px 56px; }} .article-paper {{ padding: 32px 20px; }} .article-title {{ font-size: 36px; }} p, li {{ font-size: 16px; }} .trackio-embed {{ min-height: 460px; }} }}
  </style>
</head>
<body>
  <div class="article-shell">
    <article class="article-paper">
      <header class="article-header">
        <div class="eyebrow">Trackio Report</div>
        <h1 class="article-title">{title}</h1>
        <div class="article-meta">
          <span>Generated {html.escape(str(manifest.get("generated_at", "")))}</span>
          <span>{len(ordered_pages)} source page(s)</span>
          <span>{len(manifest.get("dashboards", []))} Trackio dashboard embed(s)</span>
        </div>
      </header>
      <div class="root-content" data-report-page="{html.escape(root_page.relative_path)}">{root_page.html}</div>
      {linked_pages}
      {child_sections}
    </article>
  </div>
...
def _render_linked_pages(pages: list[ReportPage]) -> str:
    if not pages:
        return ""
    links = "\n".join(
        f"""<a class="page-link" href="#{html.escape(_page_anchor(page.relative_path))}">
