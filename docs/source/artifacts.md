...
# files are now under ./artifacts/my-model_v2/
```

By default, files are written to `./artifacts/<name>_v<version>/`; pass `root` to choose another directory. `download()` is idempotent — files already present are skipped — and when the run is backed by a Space, any file missing locally is fetched from the remote.
