...
export async function mountTheme({
  ...
  try {
    ...
    statusLine.textContent = selectedProject ? "One run, live plots, compact controls." : "Pick a project.";
    ...
    const updateSelectedRun = async (runId) => {
      ...
      metricsSubtitle.textContent = `Streaming plots for ${selectedRun.name || "the selected run"}.`;
