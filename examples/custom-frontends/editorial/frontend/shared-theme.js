...
function renderMetricCard(metric, rows) {
  ...
  const points = chartableRows.map((row, index) => {
    const x =
      padding +
      (index / Math.max(chartableRows.length - 1, 1)) * (width - padding * 2);
    const y =
      height -
      padding -
      ((row.value - min) / span) * (height - padding * 2);
    return [x, y];
  });
  ...
  card.innerHTML = `
    <h3>${metric}</h3>
    <div class="metric-meta">Latest ${formatValue(latest.value)} | ${chartableRows.length} plotted points${skippedPoints ? ` | ${skippedPoints} skipped` : ""}</div>
    ...
    <div class="metric-value">Range ${formatValue(first.value)} -> ${formatValue(latest.value)}</div>
  `;
  return card;
}
...
async function renderMetrics(element, project, run) {
  ...
  const numericSeries = rowsByMetric.filter((entry) => isChartableSeries(entry.rows));
  const otherMetrics = rowsByMetric
    .filter((entry) => !isChartableSeries(entry.rows))
  ...
  if (!numericSeries.length) {
    ...
    for (const entry of numericSeries.slice(0, 6)) {
      grid.appendChild(renderMetricCard(entry.metric, entry.rows));
    }
  ...
  if (otherMetrics.length) {
    ...
    heading.textContent = "Other Logged Items";
...
export async function mountTheme({
  ...
  try {
    ...
    projectSummary.textContent = selectedProject
      ? "A compact editorial view of one run at a time."
    ...
    if (!selectedProject) {
      ...
      selectedRunName.textContent = "No run selected";
      selectedRunMeta.textContent = "";
      return;
    }
    ...
    const updateSelectedRun = async (runId) => {
      ...
      selectedRunMeta.textContent = "";
      metricsSubtitle.textContent = `Plots rendered from /api/get_metric_values for ${selectedRun.name || "the selected run"}.`;
      const metrics = await renderMetrics(metricsEl, selectedProject, selectedRun);
      metricsCount.textContent = String(metrics);
    };
    ...
    if (metricsSubtitle) {
      metricsSubtitle.textContent = "Could not load metrics.";
    }
    if (selectedRunName) {
