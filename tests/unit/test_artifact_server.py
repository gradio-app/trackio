...
# --- full round-trip via the public endpoints ---


def test_full_round_trip(temp_dir, tmp_path, auth_bypassed, upload_consume_passthrough):
    payload = b"weights-bytes"
    digest = hashlib.sha256(payload).hexdigest()
    src = _write_src(tmp_path, "blob", payload)

    assert server.check_artifact_blobs("p", [digest])["present"] == []
    ...
    assert server.check_artifact_blobs("p", [digest])["present"] == [digest]

    log_result = server.artifact_log(
        request=auth_bypassed,
        project="p",
        name="m",
        type="model",
    ...
    assert log_result["version"] == 0

    manifest = server.get_artifact_manifest("p", "m", "latest")
    ...
    producer_lineage = SQLiteStorage.get_run_artifacts("p", "producer", "prod")
